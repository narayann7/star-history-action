...
# Render logic lives in render-charts.sh; the png output is declared in action.yml.
set -euo pipefail
cd "$(dirname "$0")"

# Fail if the render script still contains timestamped naming or the delete glob.
if grep -q 'star-history-\$theme-\$TS' render-charts.sh; then
  echo "FAIL: timestamped filename still present"; exit 1
fi
if grep -q 'TS="\$(date -u' render-charts.sh; then
  echo "FAIL: timestamp variable still present"; exit 1
fi
if grep -q -- '-name "star-history-\$theme-\*\.svg"' render-charts.sh; then
  echo "FAIL: delete-old glob still present"; exit 1
fi
# Require the stable png wiring in the render script.
grep -q 'star-history.png' render-charts.sh || { echo "FAIL: no png in render script"; exit 1; }
grep -q 'png=' render-charts.sh || { echo "FAIL: no png step output"; exit 1; }
